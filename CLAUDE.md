...
### Resuming a paused task: feedback vs. resume

`neo_pause_task` causes `neo_task_status` to report `WAITING_FOR_FEEDBACK`. `neo_send_feedback` is accepted in that state — sending feedback resumes the task and delivers the message in the same call. This is the preferred tool whenever there are new instructions, a correction, or a clarification to give — including the common case of pausing a digressing task specifically in order to redirect it.

**Dev note:** live-testing `neo_pause_task` against the backend showed `GET /v2/thread/status/{thread_id}` reporting `WAITING_FOR_FEEDBACK`, not a literal `PAUSED` string, immediately after pausing (stable across repeated polls). `PAUSED` does exist elsewhere in the codebase (`backend_poller.py`'s daemon-side `_ACCEPTED_STATUSES` gate) and may still surface under different conditions (auto_mode, timing, task type) than the simple case tested. Tool-facing text (descriptions, docs, skills) intentionally describes `WAITING_FOR_FEEDBACK` only and omits `PAUSED` — presenting two equivalent status strings as both valid was itself confusing the model into picking the wrong tool. If `PAUSED` is ever observed directly, treat it the same as `WAITING_FOR_FEEDBACK` for feedback-acceptance purposes, but don't reintroduce it into agent-facing copy without good reason.

`neo_resume_task` (`POST /v2/thread/control/{thread_id}` with `signal: RESUME`) takes no message — it only continues execution exactly as it was. Use it solely when the task should resume unchanged, with nothing new to say. Calling it when there *is* something to communicate resumes the task silently and drops that input — always prefer `neo_send_feedback` in that case.

Net effect: after `neo_pause_task`, branch on intent — `neo_send_feedback` (have something to say) vs. `neo_resume_task` (nothing to add) — rather than defaulting to `neo_resume_task` just because the status is `WAITING_FOR_FEEDBACK`.
