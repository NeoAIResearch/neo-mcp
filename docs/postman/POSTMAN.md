...
Use this guide to configure and test Neo MCP in Postman after installing `neo-mcp` 0.5.8+.
...
| **Prompts** | 5 | `train-model`, `fine-tune-classifier`, `fix-training-run`, `build-ml-pipeline`, `benchmark-prompts` |
| **Resources** | 4 | `neo://docs/overview`, `neo://docs/tools`, `neo://docs/workflow`, `neo://docs/env` |
...
1. **Prompts** → `train-model` → set `dataset_path` = `fraud.csv` → **Run** → copy message text
...
| Issue | Fix |
|-------|-----|
| Empty Prompts/Resources tabs | Upgrade to neo-mcp 0.5.8+ and **Load Capabilities** again |
