...
services:
  neo-mcp:
    ...
    read_only: true          # optional: make the container filesystem read-only
    tmpfs:
      - /tmp                 # /tmp must be writable for the health server and temp workspace
    healthcheck:
...
| `NEO_READ_ONLY` | No | `true` = expose only non-mutating status/list/test tools and reject all mutations |
