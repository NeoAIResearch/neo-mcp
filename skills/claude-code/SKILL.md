...
| `neo_send_feedback` | Neo is WAITING_FOR_FEEDBACK; to course-correct a digressing task mid-run; or to resume a paused task **with** new instructions | Resumes + delivers the message in one call. Call `neo_task_status` after sending to confirm resume |
| `neo_pause_task` | User asks to pause | `neo_task_status` will then report WAITING_FOR_FEEDBACK — afterward use `neo_send_feedback` if there's something to say, else `neo_resume_task` |
| `neo_resume_task` | User asks to resume with **no** new instructions | Works once status is WAITING_FOR_FEEDBACK. If the user has anything to add/correct, use `neo_send_feedback` instead — `neo_resume_task` resumes silently and any instructions would be lost |
...
- **Already paused and need to course-correct? Use `neo_send_feedback`, not `neo_resume_task`.** `neo_pause_task` causes `neo_task_status` to report `WAITING_FOR_FEEDBACK` — sending `neo_send_feedback` to that thread resumes the task and delivers the correction in the same call. Reach for `neo_pause_task` → `neo_resume_task` only when the user wants the task frozen and later continued with nothing new to say. If there's anything to add or correct, skip `neo_resume_task` entirely and go straight to `neo_send_feedback` — it does both jobs at once, and a separate `neo_resume_task` call afterward would resume with the correction never delivered.
