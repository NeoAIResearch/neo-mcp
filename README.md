...
| Tool | When to use |
|---|---|
| `neo_submit_task` | Start a task. Returns `thread_id` immediately. |
| `neo_list_tasks` | **Reconnect after closing Claude Code.** Lists all known tasks (from previous sessions too) with live status — RUNNING, COMPLETED, WAITING_FOR_FEEDBACK, etc. Use the returned `thread_id`s to pick up where you left off. |
...
| `neo_pause_task` | Pause a running task mid-execution (resumable). |
| `neo_resume_task` | Resume a paused task. |
| `neo_stop_task` | Permanently stop and clean up a task. Irreversible. |
...
neo_submit_task  →  thread_id
      ↓
neo_task_status  →  poll until COMPLETED / WAITING_FOR_FEEDBACK
      ↓
neo_get_messages →  full output when COMPLETED
```
...
**Returning to a session (after closing Claude Code):**
```
neo_list_tasks   →  see all tasks with live status + thread_ids
neo_task_status  →  check the one you care about
